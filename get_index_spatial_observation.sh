...
hadoop fs -cat data/$dataname > /tmp/spatialdata 1>&2
...
hadoop jar $DATA_POLYGAMY_JAR edu.nyu.vida.data_polygamy.pre_processing.PreProcessing -m $m -n $n -dn filtereddata -dh filtereddata.header -dd filtereddata.defaults -t hour -s nbhd -cs points -i $5 $6 $7 1>&2
