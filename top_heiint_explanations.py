...
def top_explanations(observation_index, header_filename, dataset_name, zones):
    ...
    for i, attribute in enumerate(attributes):
        ...
        explanation_file = open('%s/results/heiint/nonspatial_spatial/%s/%s_%s.csv'
        ...
        for line in explanation_file:
            ...
            explanations.append({
                'zones': [int(x) for x in explanation_fields[1].split('|')],
                'attribute': attribute,
                'attribute_id': i,
                'observation_value': float(explanation_fields[2]),
                'start': float(explanation_fields[4]),
                'end': float(explanation_fields[5])
            })
