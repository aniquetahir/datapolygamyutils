...
def top_nonspatial_explanations(observation_index, header_filename, dataset_name):
    header_file = open(header_filename, 'r')
    ...
    for i, attribute in enumerate(attributes):
        ...
        # Get file for observation_attribute/attribute
        explanation_file = open('%s/results/intervention/nonspatial/%s/%s_%s.csv'
                                % (scripts_directory, dataset_name, observation_attribute, attribute), 'r')
        ...
        for line in explanation_file:
            ...
            explanations.append({
                'attribute': attribute,
                'attribute_id': i,
                'observation_value': float(explanation_fields[0]),
                'start': float(explanation_fields[2]),
                'end': float(explanation_fields[3])
            })
